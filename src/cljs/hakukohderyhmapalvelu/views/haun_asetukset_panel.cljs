...
(defn- haun-asetukset-aikavali [{:keys [id-prefix value on-change required? disabled?]}]
...
(defn- haun-asetus-aikavali-container [{:keys [haku-oid required? haun-asetus-key]}]
  (let [id-prefix (get-id-prefix haun-asetus-key)
        ...
        parsed-orig-value {:start (some-> (get orig-value :start)
                                          d/long->date
                                          d/date->iso-date-time-local-str)
                           :end   (some-> (get orig-value :end)
                                          d/long->date
                                          d/date->iso-date-time-local-str)}]
    [:<>
     [haun-asetukset-label
