...
(deftest hakukohderyhma-delete-service-test
  ...
  (deftest hakukohderyhma-simple-test
    (let [datetime-formatter (DateTimeFormatter/ofPattern "yyyy-MM-dd'T'HH:mm:ss")
          ...
          latest (-> (LocalDateTime/parse "2024-05-01T16:00:00" datetime-formatter)
                     (.atZone zone-id)
                     (.toOffsetDateTime))
          second (-> (LocalDateTime/parse "2024-05-01T12:00:00" datetime-formatter)
                     (.atZone zone-id)
                     (.toOffsetDateTime))
          oldest (-> (LocalDateTime/parse "2024-05-01T11:00:00" datetime-formatter)
                     (.atZone zone-id)
                     (.toOffsetDateTime))
